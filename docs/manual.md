...
> **絶対ルール: あらゆるプロンプト送信の前後に必ず 150/24h ペース（576〜640秒）のインターバルを挟む。リトライ・Cloudflare・タイムアウト・エラー・DL失敗・復旧処理、いかなる場合も例外なし。**

- **3時間ローリングウィンドウ**で40〜50リクエストが上限
- 150req/24h = 576秒/req → 3hあたり約19リクエスト（安全圏）
- `INTER_REQUEST_SLEEP_MIN = 576`（スクリプト定数で強制）
