...
> **絶対ルール: あらゆるプロンプト送信の前後に必ず 380〜460秒のインターバルを挟む。リトライ・Cloudflare・タイムアウト・エラー・DL失敗・復旧処理、いかなる場合も例外なし。**

- **3時間ローリングウィンドウ**で40リクエストが上限
- 380〜460s/req → ~206枚/日・26req/3h（安全圏・余裕35%）
- `SEND_INTERVAL = 380`（スクリプト定数で強制）
